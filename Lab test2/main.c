...
//NODE* a= root->child->sibling->sibling;
//a= search('A',a);
//printf("\n%c",a->c);

insert_word("UTKARSH");
printlist(root->child->sibling->sibling->child);
printf("\n%d\n",search_word("UTKARSH"));
 traverse_tree(root);
 printf("\n%d\n",search_word("BAD"));
 */
...
//BADGE, , BED, ,
//BEDROCK, CONFIRM, CONFORM, FULL, CORNER, DAMN, DAM, DAMAGE, BAD, DEN, MAN, MEN,
//MENTOR
//insert_word(words[0]);
/*
...
NODE *nb = create_node('B');
    ...
    nbad->stop_word = TRUE;
    printf("%d",search_word("BAD"));
    //traverse_tree(root);
    insert_word("BADGE");
    insert_word("COMFORT");
    printf("%d",search_word("COMFORT"));
    printlist(root->child->child->child);

    //insert_word culpurit...
